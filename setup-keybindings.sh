...
EOF
		...
		if grep -Eq '^command\s*=' "$CLIPBOARD_CONFIG"; then
			sed -i 's/^command\s*=.*$/command = '"'"'wl-copy \&\& hyprctl dispatch sendshortcut "SHIFT, Insert, activewindow"'"'"'/' "$CLIPBOARD_CONFIG"
		else
			echo 'command = '"'"'wl-copy && hyprctl dispatch sendshortcut "SHIFT, Insert, activewindow"'"'"'' >>"$CLIPBOARD_CONFIG"
		fi
...
setup_hypr_keybindings() {
	...
	# Step 1.5: Replace OCR command renamed in Omarchy 3.8.x (-zh variant removed)
	if grep -q "omarchy-capture-text-extraction-zh" "$HYPR_BINDINGS"; then
		info "發現舊版 OCR 命令，正在修復..."
...
# OCR text extraction\
bindd = ALT SHIFT, O, Extract text (OCR), exec, env OMARCHY_OCR_LANGS=eng+chi_tra omarchy-capture-text-extraction\
' "$HYPR_BINDINGS"
	fi
